...
const AdminSidebar = memo(function AdminSidebar() {
	...
	const menuItems = [
		{ label: t('Accueil'), icon: LayoutDashboard, path: '/admin' },
		...
		<>
			...
			>
				...
				<nav className="flex-1 overflow-y-auto py-4">
					<div className={`flex w-full min-w-0 flex-col gap-1 space-y-1 ${
													isCollapsed ? 'flex items-center px-0' : 'px-4'}`}>
						{menuItems.map((item) => {
							...
							return (
								<button
									...
									className={`group flex items-center text-sm font-medium rounded-md transition-colors with-full justify-start space-x-2 px-3 py-2.5 
								...
								>
									<Icon className="w-5 h-5 shrink-0 transition-all duration-200" />
										{isCollapsed && (
											<span className="pointer-events-none absolute left-full ml-1 whitespace-nowrap rounded-md px-2 py-2 text-xs font-medium bg-gray-900 text-white dark:bg-blue-800 opacity-0 translate-x-1 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-150 z-50">{item.label}</span>
										)}
				...
				<div className="border-t border-gray-200 dark:border-slate-700 p-4">
					...
					<button
						onClick={toggleTheme}
						className={`w-full flex items-center justify-center gap-2 ${!isCollapsed ? 'py-2' : 'py-3' } px-3 rounded-lg text-sm font-medium text-gray-600 dark:text-slate-300 hover:bg-gray-200 dark:hover:bg-slate-700 hover:text-gray-900 dark:hover:text-white transition-all duration-200`}
