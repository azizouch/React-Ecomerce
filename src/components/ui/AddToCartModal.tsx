    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white dark:bg-slate-900 rounded-xl max-w-4xl w-full mx-4 overflow-hidden">
        <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-slate-700">
          ...
          <button onClick={onClose} className="p-1 rounded hover:bg-gray-100 dark:hover:bg-slate-800"><X className="w-5 h-5" /></button>
        </div>
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <div className="flex gap-2 overflow-auto">
              {(selectedColorId ? (colors.find(c => c.id === selectedColorId)?.images || []) : (product?.image_url ? [{ image_url: product.image_url }] : [])).map((img, idx) => (
                <button key={idx} onClick={() => setSelectedImage(img.image_url)} className="w-20 h-20 rounded overflow-hidden border border-gray-200 dark:border-slate-700">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-gray-900 mb-4">${product?.price?.toFixed?.(2) || product?.price || '0'}</p>

            <p className="text-sm text-gray-500 mb-2">Color</p>
            <div className="flex items-center gap-3 mb-4">
              {colors.length > 0 ? colors.map((c) => (
                <button key={c.id} onClick={() => handleSelectColor(c.id)} className={`w-8 h-8 rounded-full border-2 ${selectedColorId === c.id ? 'ring-2 ring-offset-2' : ''}`} style={{ backgroundColor: c.hex_code }} aria-label={c.name} />
              )) : (
            ...
            <div className="mb-4">
              ...
              <div className="flex items-center gap-4">
                <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="w-10 h-10 flex items-center justify-center bg-gray-100 rounded">-</button>
                <span className="text-lg font-semibold">{quantity}</span>
                <button onClick={() => setQuantity(quantity + 1)} className="w-10 h-10 flex items-center justify-center bg-gray-100 rounded">+</button>
              </div>
            </div>

            <div className="flex gap-3">
              <button onClick={handleAdd} className="flex-1 flex items-center justify-center gap-2 bg-blue-600 text-white py-3 rounded-lg">
